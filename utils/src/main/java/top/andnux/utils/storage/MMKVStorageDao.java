...
public class MMKVStorageDao<T> extends BaseStorageDao<T> {

    private MMKV mmkv;

    public MMKVStorageDao(Context context, Class<T> clazz) {
        super(context, clazz);
    }
    ...
    protected void loadData(SharedPreferences sp, Object t, Field field)
        ...
        if (field.getType().equals(String.class)) {
            ...
            field.set(t, value);
                ...
                field.getType().equals(boolean.class)) {
            Boolean value = mmkv.getBoolean(name, false);
            field.set(t, value);
        } else if (field.getType().equals(Date.class)) {
            String value = mmkv.getString(name, "");
            try {
